...
public class MergeAdapterController implements NestedAdapterWrapper.Callback {
    ...
    public MergeAdapterController(MergeAdapter mergeAdapter, MergeAdapter.Config config) {
        ...
        } else if (config.stableIdMode == MergeAdapter.Config.StableIdMode.SHARED_STABLE_IDS) {
    ...
    public boolean addAdapter(int i, RecyclerView.Adapter<RecyclerView.ViewHolder> adapter) {
        ...
        if (nestedAdapterWrapper.getCachedItemCount() > 0) {
            this.mMergeAdapter.notifyItemRangeInserted(countItemsBefore(nestedAdapterWrapper), nestedAdapterWrapper.getCachedItemCount());
        }
    ...
    public boolean removeAdapter(RecyclerView.Adapter<RecyclerView.ViewHolder> adapter) {
        ...
        this.mMergeAdapter.notifyItemRangeRemoved(countItemsBefore, nestedAdapterWrapper.getCachedItemCount());
        for (WeakReference<RecyclerView> weakReference : this.mAttachedRecyclerViews) {
            RecyclerView recyclerView = weakReference.get();
            if (recyclerView != null) {
    ...
    public void onItemRangeChanged(NestedAdapterWrapper nestedAdapterWrapper, int i, int i2) {
        this.mMergeAdapter.notifyItemRangeChanged(i + countItemsBefore(nestedAdapterWrapper), i2);
    }

    @Override // androidx.recyclerview.widget.NestedAdapterWrapper.Callback
    public void onItemRangeChanged(NestedAdapterWrapper nestedAdapterWrapper, int i, int i2, Object obj) {
        this.mMergeAdapter.notifyItemRangeChanged(i + countItemsBefore(nestedAdapterWrapper), i2, obj);
    }

    @Override // androidx.recyclerview.widget.NestedAdapterWrapper.Callback
    public void onItemRangeInserted(NestedAdapterWrapper nestedAdapterWrapper, int i, int i2) {
        this.mMergeAdapter.notifyItemRangeInserted(i + countItemsBefore(nestedAdapterWrapper), i2);
    }

    @Override // androidx.recyclerview.widget.NestedAdapterWrapper.Callback
    public void onItemRangeRemoved(NestedAdapterWrapper nestedAdapterWrapper, int i, int i2) {
        this.mMergeAdapter.notifyItemRangeRemoved(i + countItemsBefore(nestedAdapterWrapper), i2);
    }

    @Override // androidx.recyclerview.widget.NestedAdapterWrapper.Callback
    public void onItemRangeMoved(NestedAdapterWrapper nestedAdapterWrapper, int i, int i2) {
        int countItemsBefore = countItemsBefore(nestedAdapterWrapper);
        this.mMergeAdapter.notifyItemMoved(i + countItemsBefore, i2 + countItemsBefore);
    }
    ...
    private void calculateAndUpdateStateRestorationPolicy() {
        ...
        if (computeStateRestorationPolicy != this.mMergeAdapter.getStateRestorationPolicy()) {
            this.mMergeAdapter.internalSetStateRestorationPolicy(computeStateRestorationPolicy);
        }
    }
    ...
    public void onViewRecycled(RecyclerView.ViewHolder viewHolder) {
        NestedAdapterWrapper remove = this.mBinderLookup.remove(viewHolder);
        if (remove == null) {
    ...
    public int getLocalAdapterPosition(RecyclerView.Adapter<? extends RecyclerView.ViewHolder> adapter, RecyclerView.ViewHolder viewHolder, int i) {
        ...
        if (countItemsBefore < 0 || countItemsBefore >= nestedAdapterWrapper.adapter.getItemCount()) {
