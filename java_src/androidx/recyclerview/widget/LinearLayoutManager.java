...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
        ...
        if (focusedChild != null && anchorInfo.isViewValidAsAnchor(focusedChild, state)) {
            ...
            if (findReferenceChildClosestToStart != null) {
                ...
                if (!state.isPreLayout() && supportsPredictiveItemAnimations()) {
                    int decoratedStart = this.mOrientationHelper.getDecoratedStart(findReferenceChildClosestToStart);
                    int decoratedEnd = this.mOrientationHelper.getDecoratedEnd(findReferenceChildClosestToStart);
    ...
    View findReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State state, int i, int i2, int i3) {
        ...
        while (i != i2) {
            ...
            if (position >= 0 && position < i3) {
                ...
                } else if ((decoratedStart < endAfterPadding || decoratedEnd <= endAfterPadding) && (decoratedEnd > startAfterPadding || decoratedStart >= startAfterPadding)) {
                    return childAt;
                } else {
                    if (view == null) {
                        view = childAt;
    ...
    View findOneVisibleChild(int i, int i2, boolean z, boolean z2) {
        ensureLayoutState();
        int i3 = z ? MessageID.INF_ZONE_SMALL : 320;
        int i4 = z2 ? 320 : 0;
        if (this.mOrientation == 0) {
    ...
    View findOnePartiallyOrCompletelyInvisibleChild(int i, int i2) {
        ...
        if (this.mOrientationHelper.getDecoratedStart(getChildAt(i)) < this.mOrientationHelper.getStartAfterPadding()) {
            i3 = MessageID.MSG_REF_HOR_SCALE;
            i4 = 16388;
        } else {
            i3 = 4161;
