...
class FloatingActionButtonImplLollipop extends FloatingActionButtonImpl {
    ...
    void onElevationsChanged(float f, float f2, float f3) {
        if (Build.VERSION.SDK_INT == 21) {
            ...
            arrayList.add(ObjectAnimator.ofFloat(this.view, Key.ELEVATION, f).setDuration(0L));
    ...
    private Animator createElevationAnimator(float f, float f2) {
        AnimatorSet animatorSet = new AnimatorSet();
        animatorSet.play(ObjectAnimator.ofFloat(this.view, Key.ELEVATION, f).setDuration(0L)).with(ObjectAnimator.ofFloat(this.view, View.TRANSLATION_Z, f2).setDuration(100L));
        animatorSet.setInterpolator(ELEVATION_ANIM_INTERPOLATOR);
        return animatorSet;
    }
